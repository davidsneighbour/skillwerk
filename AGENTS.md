...
Apparatus is a collection of standalone AI skills for operating browsers, diagnostics, media utilities, and other reusable technical machinery. There is no app or build output; the repository product is the Markdown and scripts under `skills/`. Each `skills/<skill-name>/` directory is independently loadable and uses `SKILL.md` as its entrypoint. Supporting files belong inside the owning skill directory, commonly in `resources/` or `scripts/`.

## Skill map

* `web-screenshot` captures deterministic screenshots of web pages with a requested viewport, colour scheme, full-page or fixed-height behaviour, and output format. It runs through a shared machine-local browser runtime bootstrapped by `skills/web-screenshot/scripts/runtime.mjs`.

## Build, test, and development commands

There is no root build step. Run a skill's helper scripts directly from the repository root, for example:

```bash
./skills/web-screenshot/scripts/web-screenshot --help
```
...
Skill directories use lowercase hyphenated names such as `apparatus` or, for
future companion skills, `apparatus-<purpose>`; resource scripts use
